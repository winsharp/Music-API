...
export const getArtistReleases = async (id: string | number, page?: number): Promise<ArtistReleasesResult> => {
    ...
    // Narrow down to the artist's own releases (role "Main", not a guest
    // spot/remix/compilation appearance), matching "what albums they've
    // released." Entries come back as either a concrete "release" or a
    // "master" grouping — masters have no /releases/{id} page of their own,
    // so they're resolved to their designated main_release id instead of
    // being dropped (dropping them silently hid real releases: newer or
    // multi-pressing albums are often catalogued as masters, not releases).
